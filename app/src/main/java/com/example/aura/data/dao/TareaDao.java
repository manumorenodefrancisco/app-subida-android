...
public interface TareaDao {
    ...
    //contador del Home
    @Query("SELECT COUNT(*) FROM tareas WHERE aceptada = 1 AND fechaLimite >= :fechaActual")
    int contarVigentes(String fechaActual);

    @Query("SELECT COUNT(*) FROM tareas WHERE aceptada = 1 AND fechaLimite >= :fechaActual AND completada = 1")
    int contarCompletadasVigentes(String fechaActual);

    @Query("SELECT * FROM tareas WHERE aceptada = 1 AND fechaLimite >= :fechaActual AND completada = 0 ORDER BY fechaLimite ASC LIMIT 2")
